    ...
    <div className="space-y-3.5 pb-28 px-4 pt-2 max-w-md mx-auto select-none">
      ...
      <div className="flex items-center justify-between pt-1">
        <div className="space-y-0.5">
          <span className="text-[11px] font-bold text-[#1A3EEA] uppercase tracking-wider flex items-center gap-1.5">
            <ClockAlert className="w-3.5 h-3.5 text-[#1A3EEA]" />
            48-HOUR RADAR
          </span>
          <h2 className="text-2xl font-black text-[#0F172A] tracking-normal">Expiring Soon</h2>
        </div>
        <span className="px-3 py-1 rounded-full bg-[#EBF1FF] text-[#1A3EEA] text-xs font-black tracking-wide">
      ...
      {expiringMembers.length === 0 ? (
        <div className="text-center py-16 px-4 bg-white border border-[#E9ECEF] rounded-[28px] shadow-apple-card space-y-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-[#64748B] mt-1 max-w-xs mx-auto tracking-wide">
          ...
          {expiringMembers.map((member, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`w-full bg-white border rounded-[26px] p-4 sm:p-5 shadow-apple-card cursor-pointer active:scale-[0.98] transition-all relative overflow-hidden group ${
                  isPriorityTest ? 'border-[#1A3EEA] ring-2 ring-[#1A3EEA]/20' : 'border-[#E9ECEF] hover:border-[#1A3EEA]/40'
                }`}
              >
                ...
                <div className="pl-2 space-y-3.5">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3.5">
                      ...
                      <div className="space-y-0.5">
                        <div className="flex items-center space-x-2">
                          ...
                          {isPriorityTest && (
                            <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-[#1A3EEA] text-white tracking-wide">
                              TEST
                            </span>
                          )}
                        </div>
                        <p className="text-xs text-[#64748B] tracking-wider font-medium">
                    ...
                    <div className="flex items-center space-x-1.5">
                      <span className="px-3 py-1 rounded-full text-xs font-bold bg-[#EBF1FF] text-[#1A3EEA] tracking-wide">
                        {countdownText}
                      </span>
                  ...
                  <div className="flex items-center justify-between pt-2.5 border-t border-[#E9ECEF]">
                    <span className="text-xs text-[#64748B] font-medium tracking-wide">
                      Expires <span className="font-semibold text-[#0F172A]">{formatDisplayDate(member.expiry_date)}</span>
                    </span>

                    <button
                      onClick={(e) => handleSendReminder(e, member)}
                      className={`h-10 px-4 rounded-xl text-xs font-bold flex items-center space-x-2 transition-all active:scale-95 border tracking-wide cursor-pointer ${
                        isSent
